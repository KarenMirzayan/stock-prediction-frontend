...
import { LucideAngularModule, BookOpen, Search, Brain, Play, CheckCircle2, XCircle, ArrowRight, Lightbulb } from 'lucide-angular';
            ...
            <div class="p-6">
              <div class="flex items-center justify-between">
                <span class="rounded-md border border-border px-2 py-0.5 text-xs font-medium">
                  Question {{ selectedQuestion() + 1 }} of {{ data.quizQuestions.length }}
                </span>
                <lucide-icon [img]="Brain" [size]="20" class="text-accent"></lucide-icon>
              </div>
              <h3 class="mt-4 text-xl font-semibold">{{ currentQuiz().question }}</h3>
            </div>
                ...
                <div class="mt-4 rounded-lg border p-4"
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p class="mt-1 text-sm text-muted-foreground">{{ currentQuiz().explanation }}</p>
                    </div>
                  </div>
                </div>
...
export class EducationComponent {
  ...
  readonly currentQuiz = computed(() => this.data.quizQuestions[this.selectedQuestion()]);

  readonly isCorrect = computed(() => this.selectedAnswer() === this.currentQuiz().correctAnswer);
  ...
  nextQuestion(): void {
    this.selectedQuestion.update(prev => (prev + 1) % this.data.quizQuestions.length);
    this.selectedAnswer.set(null);
    this.showResult.set(false);
  }

  getOptionClass(index: number): string {
    if (this.showResult() && index === this.currentQuiz().correctAnswer) {
